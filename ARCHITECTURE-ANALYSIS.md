...
|---|---|---|
| 1 | Minimal correctness test harness | ✅ **Shipped and exceeded.** Option A of `TASK-TEST-HARNESS.md`: `calc.js` extracted, loadable identically in Node and the browser. Scoped at 8 pure functions, now **15**; scoped at "hand-written assertions", now **100**, including an independent bisection oracle for `calcEMI` (so a *formula* bug, not just a regression, is caught) and conservation properties. It has since caught real bugs — see below. |
| 2 | Basic client-side error visibility | ✅ **Shipped, 2026-08-11 (Phase 16/R66).** The cheap option, as this document recommended: `window.onerror` **and** `unhandledrejection` (both — a rejected promise is invisible to the first alone), installed as the first statement in `index.html`'s inline `<script>` so init-time failures are covered too. Surfaces a dismissible `#err-panel` (not the existing `showToast()` — diagnostics need to sit on screen long enough to read and copy, a toast auto-hides in 3.2s) naming `BUILD_STAMP`, the message, source/line/column, and the stack when available, with a "Copy diagnostics" button. Deduped by error identity (repeat count, not a stacked pile) so an exception thrown on every keystroke can't storm the screen; every DOM-touching step is wrapped in its own try/catch with a `window.alert` fallback, so the handler itself cannot throw or re-enter. ⚠️ **Never sends anything anywhere** — no `fetch`/`XMLHttpRequest`/`sendBeacon`/tracking pixel, verified by grep (`fetch(\|XMLHttpRequest\|sendBeacon\|new Image(` matches only the comment stating the rule) — the About page's "you'll see no request leave this page" claim still holds. See `CLAUDE.md`'s "Client-side error visibility" section and manual checklist item 60. Recommendation #4 below remains the one architecture mitigation **not** taken — the Phase 16 brief was explicit that only one of the two should ship. |
...
| 4 | Light modularization, one hub at a time | ❌ **Not taken — by decision now, not by default.** `calc.js` was extracted (as a side effect of #1, which this document predicted: *"one change serves two purposes"*), but the inline `<script>` was never split. There have been four natural entry points since — Worth, Compare Cars, financing modes, the absorbed loan detail — and none was taken. Phase 16/R66 explicitly chose recommendation #2 (above) over this one — "pick one, not both, not a rewrite" — so as of 2026-08-11 this stays **deliberately open**: still worth doing eventually, still not urgent, and now a named decision rather than a thing nobody got around to. |
...
**The two mitigations were prescribed together, and for thirteen phases neither was taken.** Recommendation #4 was explicitly ranked last and called *"optional, lower urgency than 1–3"* — not doing it was defensible on its own terms. Not doing it *while also* skipping #2 was weaker, because #2 was the cheap compensating control for exactly the risk #4 addresses: a silent failure in a large file with no type checker, no linter, and no build step to warn. *(Update, 2026-08-11 — Phase 16/R66.)* **#2 has since shipped**, closing that specific gap: a JS exception anywhere in the app now surfaces on screen with enough detail to be actionable, instead of a silently dead calculator. #4 remains open, but — see the table above — as a stated decision ("pick one, not both") rather than a thing skipped by default. The risk #4 addresses (global-namespace collisions, review friction in a 5,600-line file) is unchanged and still real; it just no longer compounds with a total absence of failure visibility the way it did when this section was first written.
