...
- `sw.js` — service worker: network-first for the HTML shell (so deploys show up immediately), cache-first for everything else. Cache name is versioned (`apt-cost-v9`); bump it when shipping a change that should bust old caches.
