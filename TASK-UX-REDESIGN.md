...
## Remaining work (as of 2026-08-15, after Phase 18)

R1–R4, R6, R11–R20, R5/R7/R9 (Phase 4), R26–R30 (Phase 4b), R33–R35/R37 (Phases 8 and 8b), R38–R44 (Phase 9), R45–R48 (Phase 10), R10, R21/6a, R22/6b, R24/6d + 6d-i, R45/6e-ii (Phases 5–6, Waves 0–1), R49–R54 (Phase 13), R55–R61 (Phase 14), R62–R66 (Phases 15–16), R8 and R25's orientation-line half (Phase 17), and now **R23 (Phase 18)** have all shipped (see the phase reports named throughout this table). R32/7b has been assessed (recommendation: state-and-don't-model, not build — see Phase 7) but nothing was built, per its own acceptance criteria. **B10 is answered** (2026-08-15, default-off in Grow / structural in comparisons — this document's own recorded lean). What's left is the feedback composer half of **R25**/6e (deliberately deferred past the beta, not an oversight), **R31**/7a (Phase 7, unblocked now that B10 is answered and 6c/R23's term-definition work is done, not yet started), and **6f** (the beta protocol — owner activity, not code, and now the only item on this list that isn't either deliberately deferred or a build task). Phase numbers link to the full specs below.
...
| ~~R23~~ | ✅ **SHIPPED 2026-08-15** (`PHASE-18-REPORT.md`). ~~**Inline `ⓘ` term definitions** — ~12 terms (perquisite, pre-EMI, step-up, CAGR, corpus, floor rise, IRDAI depreciation …). The highest-value comprehension work: the risk in this app is misreading an *output*, not failing to operate an input.~~ One reusable component (`.term-info`/`toggleTermInfo()`) at 17 placements across every hub, reusing R8's `aria-expanded` convention on the trigger rather than a second pattern. **Built directly, not via a worktree subagent** — a third consecutive stale-worktree base was detected (this time by the subagent itself, which correctly refused to build rather than repeat the R25 mistake; see the status header above). **`position:fixed` with JS-computed coordinates, not `position:absolute`** — found before shipping, not after: nearly every placement sits inside a `.panel-card`/`.collapse-card`, both of which set `overflow:hidden`, so an absolutely-positioned popover would have been silently clipped the moment it extended past its own ancestor. See CLAUDE.md's new **Inline term definitions** section and manual checklist item 63.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | 6     | —               | High                   | Medium     |
...
### Phase 6 — Ready for someone who isn't you (beta, then public) — 🟡 MOSTLY SHIPPED (**R21**/6a, **R22**/6b, **R24**/6d + 6d-i, **R45**/6e-ii ✅ shipped 2026-08-08 across Waves 0–1; **R25**/6e's orientation line ✅ shipped 2026-08-15 (`PHASE-17-REPORT.md`); **R23**/6c ✅ shipped 2026-08-15 (`PHASE-18-REPORT.md`); the feedback composer half of R25/6e (deliberately deferred) and 6f (beta protocol, not code) still open; D13, plus D12's framing half as 6d-i)
...
**6c — Inline `ⓘ` term definitions (R23) — ✅ SHIPPED 2026-08-15 (`PHASE-18-REPORT.md`)**
