...
## Remaining work (as of 2026-08-10, after the mid-project review)

R1–R4, R6, R11–R20, R5/R7/R9 (Phase 4), R26–R30 (Phase 4b), R33–R35/R37 (Phases 8 and 8b), R38–R44 (Phase 9), R45–R48 (Phase 10), R10, R21/6a, R22/6b, R24/6d + 6d-i, R45/6e-ii (Phases 5–6, Waves 0–1), R49–R54 (Phase 13), and R55–R61 (Phase 14) have all shipped (see `PHASE-3-REPORT.md`, `PHASE-3B-REPORT.md` — covering 3b and 3c — `PHASE-4-REPORT.md`, `PHASE-4B-REPORT.md`, `PHASE-8-REPORT.md`, `PHASE-8B-REPORT.md`, `PHASE-9-REPORT.md`, `PHASE-10-REPORT.md`, `PHASE-11-REPORT.md`, `PHASE-12-REPORT.md`, `PHASE-13-REPORT.md` and `PHASE-14-REPORT.md`). R32/7b has been assessed (recommendation: state-and-don't-model, not build — see Phase 7) but nothing was built, per its own acceptance criteria. What's left is **R8** (Phase 5, not yet started), **R23**/6c and the rest of **R25**/6e (Phase 6), Phase 7 (R31, gated on B10), and the five items **R62–R66** filed by the 2026-08-10 mid-project review. Phase numbers link to the full specs below.
...
| R8      | **Keyboard & ARIA pass** (D8) — clickable `<div>`s become `<button>`s with `aria-expanded`, `role="tablist"`/`aria-selected` on both tab bars, ≥44px touch targets, `:focus-visible` everywhere. Phase 3 added more collapse headers (loan/advanced/SIP compare cards) that need this pass too. **Phase 3b raised the stakes:** `#btn-detail`/`#btn-loan`/`#btn-disb` are real `<button>`s but carry no `aria-expanded`, and the R14 accordion now silently collapses a panel the user *didn't* click — a state change on a second control with no accessible signal at all. Add `aria-expanded` in `toggleSection()`'s open, close, and sibling-collapse paths together. **⚠️ RE-RATED Medium → High on 2026-08-10** (`MID-PROJECT-REVIEW.md` §5.2): the Medium rating predates B4 being answered *"general audience"* on 2026-07-25 and was never revisited afterwards. Measured state on that date — `aria-expanded`: **0** occurrences in `index.html`; `role="tablist"`: **0**; `aria-selected`: **0**; `tabindex`: **0**; `min-height:44px`: **0**; `:focus-visible`: **3** rules; **15** `<div class="collapse-header">` still not buttons. **This item grows with every phase that isn't it** — Phase 14 alone added four more collapsible surfaces (`#cc-tenure-card`, `#cc-crossmode-card`, and both reveals).                                                                                                                                                                 | 5     | —                               | **High** (was Medium)  | Medium     |
...
| R25     | 🟡 **Partially shipped 2026-08-08** (Wave 1, see `PHASE-12-REPORT.md`): ~~a visible contact link (About page and landing footer)~~ — done, as a plain `mailto:` link per 6e-i's sequencing decision. **Still open:** the dismissible first-run orientation line on the landing page, and (explicitly deferred until before public release, not this beta, per 6e-i) the local feedback composer. With analytics ruled out by design, an unsolicited message is the *only* feedback channel that will ever exist. Full reasoning and the disqualified options (embedded forms, Google Forms) in 6e.                                                                                                                                                                                                                                                     | 6     | R45 for the build stamp (✅ shipped) | Medium                 | Low–Medium |
...
| **R62** | 🐛 **D7's iOS-zoom fix has regressed — restore its selector list.** *(Live defect, found 2026-08-10, `MID-PROJECT-REVIEW.md` §6.1.)* `index.html:505`'s `@media(max-width:600px)` block carries the D7 fix **and** a comment saying *"Every new input class must be added here too"* — Phase 0 flagged this as a **standing obligation, not a one-off**. Two later phases added input classes and neither was added: **`.cc-field input, .cc-field select`** (`index.html:692`, **14px**, shipped Phase 9 and extended by Phase 14 — every Compare Cars shortlist field including Down Payment, plus the `.cc-field` assumptions inputs) and **`.qf select`** (`index.html:181`, **15px**, shipped Phase 6a — `q-state`, i.e. the control that makes the app correct for non-Telangana users). On an iPhone, tapping any of these lurches the viewport. **Fix is one line**: add those three selectors. *Do the same-commit hardening too* — restate the comment as a checkable rule and add a checklist item that greps for input rules under 16px, since a comment alone has now failed twice. | **15** | — | **High (live bug)** | **Trivial** |
| **R63** | 🐛 **Dhanam Grow states nothing about tax, anywhere — ship 7a's copy-only caveat.** *(Live gap, found 2026-08-10, `MID-PROJECT-REVIEW.md` §6.2.)* Phase 7b's own assessment concluded **state-and-don't-model** and ended with *"Ship 7a's copy-only caveat (and 6d-i's, which already exists) instead."* 6d-i shipped; **this did not.** `grep -in "tax"` across `hub-sip` (`index.html:1917–2103`) returns **zero** matches — a user opening the app's **first landing tile**, typing ₹50,000/mo over 20 years, reads a ₹4.98 Cr corpus with nothing on screen saying it is pre-tax. The exact sentence to ship is already written in Phase 7's assessment (§4 of that block). ⚠️ **This does NOT need B10 answered** — B10 gates a *computed* post-tax figure (R31); this is a stated caveat. Untangling the two has been blocking a one-paragraph fix. Reuse the existing `.sip-caveat` component (already used twice). | **15** | — | **High (D12 precision half)** | **Trivial** |
| **R64** | 📝 **Documentation-drift sweep.** *(`MID-PROJECT-REVIEW.md` §7 carries the itemised list.)* Two items would actively mislead someone acting on them — both handled 2026-08-10 in the review pass: `TASK-TEST-HARNESS.md` and `TASK-COLOR-PALETTE.md` both still read *"Status: NOT approved for execution"* despite having shipped, and the former pinned `calcPerquisite` at the **superseded** 1800/2400/2700/3300 figures. ✅ Both fixed. **Still open:** CLAUDE.md's stale facts (line count `~3970` → 5,589; cache name `apt-cost-v14` → `apt-cost-v19`; *"Six call sites"* → 8; the retired `cb-*` still listed as a live ID convention; checklist items 22 and 41 naming `cb-depr-chart`/`renderCarLoan()`), and `UX-ANALYSIS.md`'s *"forest green + gold"* palette description. | **15** | — | Medium | Low |
| **R65** | **Close the Strategic-#2 loop, or explicitly drop it.** *(`MID-PROJECT-REVIEW.md` §2.2.)* `UX-ANALYSIS.md`'s central thesis — *"Worth is the noun; the calculators are the verbs"* — promised two specific things that were never given R-numbers and therefore never got built: *"the prepayment simulator can show its effect on **your** net worth curve, not a hypothetical loan"* and *"the verdict cards become personal advice instead of generic comparisons."* Neither `renderAdvLoan()` nor `renderSIPComparison()` reads `DS`. The bridge runs **one way only** (Worth reads the calculators' engines; no calculator reads Worth), so the most-argued thesis in the analysis is **one-third delivered**. Both candidates are small — the persistence layer already exists. ⚠️ **If we don't want them** (reasonable — they add the cross-hub coupling `ARCHITECTURE-ANALYSIS.md` §2 warned about), **write that down in `UX-ANALYSIS.md` §Strategic-2 as a decision**, so the thesis isn't left half-finished by accident. | **16** | — | Medium | Low–Medium |
| **R66** | **Take one of the two architecture mitigations — not both, not a rewrite.** *(`MID-PROJECT-REVIEW.md` §2.3.)* `ARCHITECTURE-ANALYSIS.md`'s recommendation **#2 (client-side error visibility) has been open since day one** and #4 (light modularization) was declined by default rather than by decision, while the numbers it predicted got worse: `index.html` **2,888 → 5,589** lines (it predicted 4,000–5,000), module-scope globals **~8 → 38** (9 of them Compare Cars UI state alone), inline handlers **93 → 167**. Pick one: *(cheap, recommended)* a `window.onerror`/`unhandledrejection` handler surfacing a *"something went wrong — build `<BUILD_STAMP>`, copy diagnostics"* toast — ~15 lines, pairs naturally with R25's feedback composer, and directly serves the beta, where the failure mode is *"a tester says it broke and we have nothing"*; or *(structural)* split the inline `<script>` at the next hub-sized change, `car.js` first at ~39% of churn. | **16** | — | Medium | Low (cheap option) |
...
### Phase 5 — Accessibility & trust polish (D8, D11) — 🟡 PARTIALLY SHIPPED (**R8** not started; **R10** partially shipped 2026-08-08, Wave 0 — see `PHASE-11-REPORT.md`; B4 now answered — see Phase 6)

- Convert clickable `<div>`s (collapse headers, adv/sip headers) to `<button>`s with `aria-expanded`; add `role="tablist"`/`aria-selected` to hub nav and SIP planner tabs; ≥44px touch targets for mode toggles; visible `:focus-visible` on all interactive elements. **(R8 — still open.)**
...
- Phase 2 added new interactive controls that this pass must cover: the `#l-remember` toggle, the four Worth data buttons, and the file input behind "Restore backup". **(Still open — part of R8.)**
...
- *Acceptance:* full keyboard traversal of every hub (R8, open); tables don't overflow at 375px (✅ shipped).

### Phase 6 — Ready for someone who isn't you (beta, then public) — 🟡 PARTIALLY SHIPPED (**R21**/6a, **R22**/6b, **R24**/6d + 6d-i, **R45**/6e-ii ✅ shipped 2026-08-08 across Waves 0–1; **R23**/6c, the rest of **R25**/6e (orientation line + composer), and 6f (beta protocol, not code) still open; D13, plus D12's framing half as 6d-i)
...
- ✅ **The contact-link half shipped 2026-08-08** (Wave 1, `PHASE-12-REPORT.md`): a plain `mailto:` link, both on the About page and in the new landing footer — no data collection, no prefilled app state, nothing beyond the address itself. **The dismissible orientation line has not shipped** and remains open.
...
### Phase 15 — Mid-project review remediation: two live defects and the doc sweep — ❌ NOT STARTED (**R62**, **R63**, **R64**)
...
>
> ⚠️ **Phase 15 changes no financial math.** `node tests.js` must stay at 95/95 throughout.

- **R62 — restore the D7 selector.** Add `.cc-field input, .cc-field select, .qf select` to the `@media(max-width:600px)` block at `index.html:505`. Then harden the rule that failed: the existing comment (*"Every new input class must be added here too"*) has now been silently violated by two separate phases, so a comment is demonstrably not enough. Add a CLAUDE.md manual-checklist item that names the grep (`font-size:1[0-5]px` on an input/select rule) and states the invariant — **no input or select may render below 16px at ≤600px width**.
- **R63 — place Phase 7b's caveat in `hub-sip`.** The sentence is already drafted in this document's Phase 7b assessment (§4 of that block). Put it in a `.sip-caveat` (the component exists and is used twice already) above the corpus hero. **Decide once whether it appears on all three Grow tabs or once above the tab row** — the D4 density work argues for once, the "read only the hero" test argues for per-tab; pick one and record it. Do **not** compute anything: this is the `state-and-don't-model` outcome, not R31.
- **R64 — documentation sweep.** `MID-PROJECT-REVIEW.md` §7 has the itemised table. The two "would mislead" entries were fixed during the review itself; what remains is CLAUDE.md's stale facts and `UX-ANALYSIS.md`'s palette description. **Note the `--text-faint` contradiction specifically:** CLAUDE.md correctly says no such token exists, while this document's Phase 0 and 3b-b both instruct using one. CLAUDE.md is right — the docs here are the ones to fix.
- *Acceptance:* at 375px on iOS Safari, focusing any Compare Cars field or the state selector does **not** zoom the viewport; the Grow hub states its pre-tax assumption without expanding anything; `node tests.js` unchanged at 95/95; no CLAUDE.md statement of fact (line counts, cache name, chart call sites, ID prefixes) disagrees with `index.html`.

### Phase 16 — Unfinished theses: the Worth bridge and the architecture mitigation — ❌ NOT STARTED (**R65**, **R66**)
...
- **R65 — the reverse Worth bridge, or an explicit decision against it.** See the work-table row. If building: `renderAdvLoan()` and/or `renderSIPComparison()` read `DS` for the user's real balance sheet. ⚠️ **This is exactly the cross-hub coupling `ARCHITECTURE-ANALYSIS.md` §2 warned about** — a calculator reaching into another hub's state — so if it ships, it ships through a single named accessor, not by reading `DS` fields inline in two render functions. If not building, edit `UX-ANALYSIS.md` §Strategic-2 to say so and why.
- **R66 — one architecture mitigation.** See the work-table row. The cheap option (`window.onerror`) is recommended and is a beta prerequisite in practice: without it, a tester's *"it broke"* is unfalsifiable in exactly the way `BUILD_STAMP` was introduced to fix for *"I still see the bug"*. ⚠️ **If the error handler ships, it must not send anything anywhere** — it surfaces text on screen for the user to copy, same shape and same reasoning as 6e-i's local composer. An error-reporting endpoint would forfeit the privacy claim more cheaply than a backend would.
- *Acceptance:* whichever way each is resolved, the resolution is **written down in the source document that made the original promise** (`UX-ANALYSIS.md` for R65, `ARCHITECTURE-ANALYSIS.md` for R66), not only here.
